...
class Simulation():
    ...
    def __call__(self):
        ...
        while self.simulation_state["running"]:
            ...
            if self.still_in_waitline(car_current_state=self.car.current_state):
                ...
                # Estimate the position of the car in the wait line.
                new_position = self.car.report_gps_position(self.waitline)
                self.location_points.append(new_position["lat_lon"])
                # print(new_position["shapely"])
            else:
                # Otherwise stop simulation
                self.simulation_state["running"] = False
                # Report the geojson
                print(self.generate_point_geojson())
    ...
    def still_in_waitline(self, car_current_state):
        if car_current_state["odometer"] <= self.waitline.waitline_length:
            return True
        else:
            return False
